...
const createProject = (projectName, options) => {
  ...
  // Log.d(`mkdir -p ${workspaceGopath}/bin ${workspaceGopath}/pkg ${workspaceGopath}/src`)
  exec(`mkdir -p ${workspaceGopath}/bin ${workspaceGopath}/pkg ${workspaceGopath}/src`)
...
const ensureProject = (projectName, options) => {
  ...
  exec(`gvm pkgset delete ${pkgsetName}`, opt)
  exec(`gvm pkgset create ${pkgsetName}`, opt)
  exec(`cd $GVM_ROOT/pkgsets/${goVersion}/${pkgsetName} && mkdir -p src bin pkg`, opt)
