...
const install = goVersion => {
  ...
  if (!installed) {
    ...
    try {
      ...
      console.error(`fail to install ${target} due to:`)
      console.error(e)
    }
  }
}
...
const createProject = (projectName, options) => {
  ...
  console.log(`using ${chalk.green.bold(goVersion)} to create workspace`)
  ...
  // console.log(`go.gopath: ${workspaceGopath}`)
  // console.log(`go.toolsGopath: ${globalGopath}`)
  
  const projectParentPath = path.join(workspaceGopath, 'src')
  const projectPath = path.join(workspaceGopath, 'src', projectName)
  // console.log(`projectParentPath: ${projectParentPath}`)
  ...
  console.log(`project ${chalk.green.bold(projectName)} created successfully.`)
  console.log(`script to start:`)
  const start_script = chalk.cyan.green(`code ${path.join(cwd, projectName)}` )
  console.log()
