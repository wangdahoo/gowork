...
const getGopath = (projectName, reset = false) => {
  ...
  if (!fs.existsSync(gopath)) {
    exec(`mkdir -p ${gopath}`)
    exec(`mkdir -p ${gopath}/bin ${gopath}/pkg ${gopath}/src`, opt)
  }
  return gopath
}
...
const createProject = (projectName, options) => {
  ...
  // settings.json
  fs.writeFileSync(`${vscode}/settings.json`, JSON.stringify(getSettings(projectName, GOROOT), null, '  '))
  // launch.json
  ...
  /** create empty go.mod */
  exec(`echo 'module "${projectName.replace(/[-_]/, '').toLowerCase()}"' > ${projectPath}/go.mod`, opt)
