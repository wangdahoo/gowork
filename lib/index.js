...
const GOPATH = exec('go env GOPATH', opt).trim('\n')
...
const createProject = (projectName, options) => {
  ...
  // settings.json
  fs.writeFileSync(`${vscode}/settings.json`, JSON.stringify(getSettings(projectName, GOROOT, GOPATH), null, '  '))
  // launch.json
